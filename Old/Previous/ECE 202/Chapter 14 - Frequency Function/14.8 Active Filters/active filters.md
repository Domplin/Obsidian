...
2) they can provide amplifier gain in addition to providing the same frequency response as [[Passive Filters]]
3) active filters can be combined with buffer amplifiers (voltage followers) to isolate each stage of the filter from source and load impedance effects
	- This isolation allows designing the stages independently and then cascading them to realize the desired transfer function
